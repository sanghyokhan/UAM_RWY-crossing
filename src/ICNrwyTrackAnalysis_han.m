...
for fID = 1:length(MLATlist)        % 시간별로
    ...
    for i=1:length(uniID)    % 한시간동안의 항적에서 각각의 c/s별로
        ...
        if bDep   
            ...
            % Flightplan에 어떤 데이터가 있는지 확인
            %%%%% indFP = intersect(find(strcmp(uniID(i), IIS_Dep.FLIGHT_3)), find(strcmp(datestr(FltDT(indID(indTO)), 'yyyy-mm-dd'), IIS_Dep.SDT)));    % c/s와 날짜로 indexing
            indFP = intersect(find(strcmp(uniID(i), depfp_data.FLT)), find(strcmp(datestr(FltDT(indID(indTO)), 'yyyy-mm-dd'), depfp_data.SDT)));    % 한상혁 수정
            if length(indFP)>1
                disp(['[DEP] ', uniID{i}, ' - multiple info in FP']);
            else
                indFP2 = intersect(find(strcmp(uniID(i), IIS_Dep.FLIGHT_3)), find(strcmp(datestr(FltDT(indID(indTO)), 'yyyy-mm-dd'), IIS_Dep.SDT)));
                if isempty(indFP)    % flightplan에 자료가 없는 경우 disp
                    disp(['[DEP] ', uniID{i}, ' - no info in FP']);
                elseif ~strcmp(IIS_Dep.RWY(indFP2), rwyName(depRwyID))    % IIS와 항적의 활주로가 다른 경우만 disp
                    if isnumeric(IIS_Dep.RWY{indFP2}) && isnumeric(rwyName{depRwyID})
                        if IIS_Dep.RWY{indFP2}~=rwyName{depRwyID}
                            disp(['[DEP] ', uniID{i}, ' - ', num2str(rwyName{depRwyID}), ' - FP ', num2str(IIS_Dep.RWY{indFP})]);
                        end
                    else
                        disp(['[DEP] ', uniID{i}, ' - ', rwyName{depRwyID}, ' - FP ', IIS_Dep.RWY{indFP2}]);
...
%                         text(FltLat(indID(indTO)), FltLon(indID(indTO)), ['Dep ', uniID{i}]);
        ...
        % Arrival
            ...
            if length(indFP)>1
                ...
                indFP2 = intersect(find(strcmp(uniID(i), IIS_Arr.FLIGHT_3)), find(strcmp(datestr(FltDT(indID(indLD)), 'yyyy-mm-dd'), IIS_Arr.SDT)));
                if isempty(indFP)
                    disp(['[ARR] ', uniID{i}, ' - no info in FP']);
                elseif ~strcmp(IIS_Arr.RWY(indFP2), rwyName(arrRwyID))    
                    if isnumeric(IIS_Arr.RWY{indFP2}) && isnumeric(rwyName{arrRwyID})
                        if IIS_Arr.RWY{indFP2}~=rwyName{arrRwyID}
                            disp(['[ARR] ', uniID{i}, ' - ', num2str(rwyName{arrRwyID}), ' - FP ', num2str(IIS_Arr.RWY{indFP})]);
                        end
                    else
                        disp(['[ARR] ', uniID{i}, ' - ', rwyName{arrRwyID}, ' - FP ', IIS_Arr.RWY{indFP2}]);
%                         
%                         figure;
%                         geoplot(FltLat(indID), FltLon(indID));
%                         hold on;
