    ...
    for i=1:length(uniID)
        ...
        if bDep
            ...
            if (az < 90) || (az > 270) % North flow
                depRwyID = indMinDist;
                [~, indROT1] = min(distance(rwyThrLat(depRwyID), rwyThrLon(depRwyID), curLat, curLon, wgs84Ellipsoid)); % Conservative: Rwy entrance time
                % indROT1 = indTO;    % Nonconservative: Wheel-off time
...
%                     end
            ...
            if (az < 90) || (az > 270) % North flow
                ...
                indROT2 = find(rwyDist2>50, 1); % Conservative: First point after touch down at which distance from arrival runway is larger than 50 m
                % indROT2 = indLD;    % Nonconservative: Wheel-on time
            else
...
indOnRwy12 = find(sum(onRwy == [1, 2, 4, 5], 2));   % arrival 33L/15R, 33R/15L
indOffRwy12 = find(sum(offRwy == [1, 2, 4, 5], 2)); % departure 33L/15R, 33R/15L
totalROT = [onROT(indOnRwy12, :), ones(size(indOnRwy12)); offROT(indOffRwy12, :), zeros(size(indOffRwy12))];
[~, indSortROT1] = sort(totalROT(:, 1));    % Sort by ROT1

occupiedTime = totalROT(indSortROT1(1), 1:2);  % At least one of RWY 33R/15L, 33L/15R is occupied
occupiedOps(1, 1:2) = totalROT(indSortROT1(1), 3);
nInterval = 1;
for i=2:length(indSortROT1)
    if occupiedTime(nInterval, 2) < totalROT(indSortROT1(i), 1)
        nInterval = nInterval + 1;
        occupiedTime(nInterval, 1:2) = totalROT(indSortROT1(i), 1:2);
        occupiedOps(nInterval, 1:2) = totalROT(indSortROT1(i), 3);
    elseif occupiedTime(nInterval, 2) < totalROT(indSortROT1(i), 2)
        occupiedTime(nInterval, 2) = totalROT(indSortROT1(i), 2);
        occupiedOps(nInterval, 2) = totalROT(indSortROT1(i), 3);
    end
end
freeTimeDuration = zeros(nInterval-1, 1);   % Both RWY 33R/15L, 33L/15R are free (in s)
freeTimeOps = cell(nInterval-1, 1); % Last ops before free time & First ops after free time (e.g., D-A, A-A)
...
for i=1:nInterval-1
    ...
    if occupiedOps(i, 2) == 0 && occupiedOps(i+1, 1) == 0
        freeTimeOps{i} = 'D-D';
    elseif occupiedOps(i, 2) == 0 && occupiedOps(i+1, 1) == 1
        freeTimeOps{i} = 'D-A';
    elseif occupiedOps(i, 2) == 1 && occupiedOps(i+1, 1) == 0
        freeTimeOps{i} = 'A-D';
    else    % occupiedOps(i, 2) == 0 && occupiedOps(i+1, 1) == 1
        freeTimeOps{i} = 'A-A';
    end
end

uniFTops = unique(freeTimeOps);
for i=1:length(uniFTops)
    indFTops = find(strcmp(uniFTops(i), freeTimeOps));
    disp([uniFTops{i}, ': ', num2str(length(indFTops)), ' times w/ mean ', num2str(mean(freeTimeDuration(indFTops))), ' std ', num2str(std(freeTimeDuration(indFTops))), ' max ', num2str(max(freeTimeDuration(indFTops)))]);
end
